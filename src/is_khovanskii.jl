...
"""
    ...
    3)If bool is true, a vector of monomials in KS which are the initial forms of the images $\phi_{7,3}(t)(G_{7,3})$ but with the x coordinates evaluated at one and the y called z. This is just a raw version of the initial forms which we will obtained later by calling the function initial_forms_of_sections
...
"""
    ...
    The output is a boolean which is true if and only if the Hilbert function of the algebra generated by the initial forms of $\phi_{7,3}(t)(G_{7,3})$ is equal to the Hilbert function of the the Cox ring in degree two.
